...
function getDivisorsCnt(n){
    let count = 0;
    if (n % Math.sqrt(n) == 0){  //if this is a perfect square.. this takes care of the square root
        count++;
    }
    for(let i = 0; i < Math.sqrt(n); i++){ //this takes care of the other numbers being passed in.
        if(n % i == 0){
            count += 2;
        }
    }
...
const sumDigits = number => 
    Math.abs(number).toString().split("").reduce(function(a, b) {
